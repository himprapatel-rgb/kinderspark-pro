...
└── frontend/
    ...
    │   ├── i18n.ts                ← 10-language translations (all langs in-file; SupportedLang = en|fr|es|ar|ur|hi|zh|pt|de|tr)
    │   ├── speech.ts              ← TTS engine: API-first (Google→OpenAI→Azure) with 5-min retry cooldown; exports getApiTTSStatus(), resetApiTTSCheck()
...
- **RTL support** — Arabic + Urdu: `AccessibilityProvider` sets `dir="rtl"` + `html.lang` automatically when `settings.lang` is `ar` or `ur`
...
Source of truth for iOS/Android: `translations/master.json` (sync via `node translations/sync.js`)
Web source of truth: `frontend/lib/i18n.ts` — all 10 language packs are **in-file** (not auto-generated from master.json)
...
- `SupportedLang` type = all 10 lang codes (was 5 before 2026-04-02; ar/ur/pt/de/tr added)
- `RTL_LANGS` = `Set(['ar', 'ur'])` — exported from `i18n.ts`
- `AccessibilityProvider` applies `dir="rtl"` and `html.lang` on language change
- All settings pages use `setLang()` from `useTranslation` (not `updateSettings({ lang })` directly) — ensures RTL and html.lang update immediately
- Missing translation keys fall back to English automatically in `t()` function
- When adding new UI strings: add to `i18n.ts` `en` block first, then add translations for other languages; missing keys are safe (fall through to English)
...
| TTS | `tts.service.ts` — **4-tier cascade**: Google Neural2 → OpenAI (nova/echo/shimmer) → Azure Neural → Web Speech API fallback. All results cached in `AIResponseCache` (30-day TTL). 10 language support with gender-specific voices. `speech.ts` uses 5-min retry cooldown (not permanent disable) when provider unavailable; exports `getApiTTSStatus()` / `resetApiTTSCheck()` used by child settings voice section. |
...
- TTS session lockout fixed ✅ — `speech.ts` uses 5-min cooldown instead of permanent `apiTTSAvailable = false`; auto-retries after provider keys are added without requiring page reload
...
- Language switching ✅ — all 10 languages have translation packs in `i18n.ts`; RTL applied via `AccessibilityProvider`; settings pages use `setLang()` (not direct `updateSettings`) so direction/lang attrs update instantly
...
|------|-------|-------------|
| Child | `/child/settings` | My Identity (avatar/profile links), Look & Feel (large/hc/dys toggles), Screen Time, My Voice (TTS + provider status badge + Retry), Language, Reminders |
...
- TTS human voices need env vars set on Railway: `GOOGLE_TTS_API_KEY`, `OPENAI_API_KEY`, or `AZURE_TTS_KEY` — app falls back to Web Speech API (robotic) until at least one is set. Google TTS Neural2 recommended (free 1M chars/mo). Child settings voice section shows "Device Voice" badge with Retry button when falling back.
...
- Translation depth: the 5 languages added in 2026-04-02 (ar/ur/pt/de/tr) have ~60 key translations; remaining keys fall back to English. Add more keys to `frontend/lib/i18n.ts` as needed.
...
- **Language switching**: all 10 langs have translation packs; RTL works; `AccessibilityProvider` applies `dir`/`html.lang`.
- **TTS session lockout**: no longer permanently disabled on first fallback — 5-min cooldown then retries.
