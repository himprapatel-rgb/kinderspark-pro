...
export default function MissionControlPage() {
  ...
  // Load config from public/agents-config.json
  useEffect(() => {
    fetch('/agents-config.json')
      ...
      .catch(() => {
        ...
        setConfig({ agents: [], categories: {} })
      })
  }, [])

  // Fetch conversations + memories for tabs
      ...
      es.addEventListener('update', (e) => {
        ...
        const newMsgs: FeedMsg[] = (data.runs || []).slice(0, 15).map((run: any) => {
          ...
          return {
            ...
            message: type === 'running' ? 'Running...'
              : type === 'success' ? 'Completed successfully'
              : type === 'failure' ? 'Failed — needs attention'
        ...
        const issueMsgs: FeedMsg[] = (data.issues || []).slice(0, 5).map((issue: any) => ({
          ...
          agentName: 'Health Monitor', agentIcon: '🚨', agentColor: '#FF453A',
          message: issue.title, type: 'alert' as const,
          ts: new Date(issue.created_at).getTime(), url: issue.html_url,
        }))

        // Also include memory events from feed
        const memMsgs: FeedMsg[] = (data.memories || []).slice(0, 10).map((m: Memory) => ({
          id: `mem-${m.id}`,
          agentName: m.agentName, agentIcon: m.agentIcon, agentColor: m.agentColor,
          message: `[${m.type}] ${m.content.slice(0, 80)}`,
          type: m.importance >= 4 ? 'alert' as const : 'info' as const,
          ts: new Date(m.createdAt).getTime(),
        }))

        // Agent-to-agent conversation events
        const convMsgs: FeedMsg[] = (data.conversations || []).slice(0, 5).map((c: Conversation) => ({
          id: `conv-${c.id}`,
          agentName: c.fromName, agentIcon: c.fromIcon, agentColor: c.fromColor,
          ...
          <div className="ml-auto flex items-center gap-2">
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-black"
              ...
              <span className="w-1.5 h-1.5 rounded-full"
                style={{ background: isConnected ? '#30D158' : '#FF453A', boxShadow: isConnected ? '0 0 6px #30D158' : 'none' }} />
            ...
            <>
              ...
              <div className="flex gap-2 mb-4 overflow-x-auto pb-2">
                <button
                  ...
                  style={{
                    background: activeCat === 'all' ? '#5E5CE625' : 'rgba(255,255,255,0.05)',
                    border: `1px solid ${activeCat === 'all' ? '#5E5CE650' : 'rgba(255,255,255,0.08)'}`,
                    color: activeCat === 'all' ? '#5E5CE6' : 'rgba(255,255,255,0.4)',
                  }}
                >
                  All Agents
                </button>
                  ...
                  {filteredAgents.map(agent => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {agent.desc && (
                          <p className="text-white/35 text-[10px] font-bold mb-2 leading-tight">{agent.desc}</p>
                        )}
                ...
                <div className="space-y-2">
                  {conversations.map(conv => (
                    <div key={conv.id} className="rounded-2xl p-4"
                      style={{ background: conv.resolved ? 'rgba(255,255,255,0.03)' : `linear-gradient(135deg, ${conv.fromColor}08, transparent)`,
                               border: `1px solid ${conv.resolved ? 'rgba(255,255,255,0.06)' : conv.fromColor + '25'}`,
                               opacity: conv.resolved ? 0.5 : 1 }}>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1 flex-wrap">
                            ...
                            <span className="text-white/60 text-xs font-black">
                              {conv.toAgentId === 'all' ? '📢 All Agents' : conv.toName || conv.toAgentId}
                            </span>
                            ...
                            {conv.resolved && (
                              <span className="text-[9px] font-black px-1.5 py-0.5 rounded-full"
                                style={{ background: 'rgba(48,209,88,0.15)', color: '#30D158' }}>✓ resolved</span>
                            )}
                          </div>
                          ...
                          <div className="flex items-center gap-3 mt-2">
                            ...
                            {!conv.resolved && (
                              <button onClick={() => resolveConv(conv.id)}
                ...
                <div className="space-y-2">
                  {memories.map(mem => (
                    <div key={mem.id} className="rounded-2xl p-4"
                      style={{ background: `linear-gradient(135deg, ${mem.agentColor}06, transparent)`,
                               border: `1px solid ${mem.agentColor}18` }}>
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1 flex-wrap">
                            ...
                            <span className="text-[9px] font-black px-1.5 py-0.5 rounded-full ml-auto"
                              style={{ background: importanceColor(mem.importance) + '20', color: importanceColor(mem.importance) }}>
                              {importanceLabel(mem.importance)}
                            </span>
                          </div>
                          <p className="text-white/70 text-sm font-bold leading-snug">{mem.content}</p>
                          <span className="text-white/25 text-[10px] font-bold mt-1 block">
          ...
          <div className="sticky top-4">
            ...
            <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid rgba(255,255,255,0.06)' }}>
              ...
              <div ref={feedRef} className="overflow-y-auto" style={{ maxHeight: 'calc(100vh - 320px)' }}>
                ...
                {feedMsgs.map((msg, i) => (
                  <div key={msg.id + i} className="px-3 py-3 hover:bg-white/[0.02] transition-colors"
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          {msg.url && (
                            <a href={msg.url} target="_blank" rel="noopener noreferrer"
                              className="text-[10px] font-bold" style={{ color: msg.agentColor + '80' }}>View →</a>
                          )}
                        </div>
                      </div>
                    </div>
