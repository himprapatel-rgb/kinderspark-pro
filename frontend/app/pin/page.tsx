...
const ROLE_META: Record<string, { emoji: string; label: string; grad: string; color: string; glow: string }> = {
  child:   { emoji: '🧒', label: 'Kid',     grad: 'linear-gradient(135deg,#F97316,#EA580C)', color: '#F97316', glow: 'rgba(255,159,10,0.45)' },
  teacher: { emoji: '👩‍🏫', label: 'Teacher', grad: 'linear-gradient(135deg,#6C63FF,#8B5CF6)', color: '#6C63FF', glow: 'rgba(94,92,230,0.45)' },
  parent:  { emoji: '👨‍👩‍👧', label: 'Parent',  grad: 'linear-gradient(135deg,#2DB854,#34D399)', color: '#2DB854', glow: 'rgba(48,209,88,0.45)' },
  admin:   { emoji: '⚙️', label: 'Admin',   grad: 'linear-gradient(135deg,#8B5CF6,#6C63FF)', color: '#8B5CF6', glow: 'rgba(191,90,242,0.45)' },
}
    ...
    >
      ...
      {error && (
        <div
          ...
          style={{ background: 'rgba(255,69,58,0.15)', border: '1px solid rgba(255,69,58,0.3)', color: '#EF4444' }}
        >
          ⚠️ {error}
        </div>
      )}
