...
function PinEntry() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-5"
      ...
      {/* Back button */}
      <button onClick={() => router.back()} className="absolute top-5 left-5 text-white/60 text-sm font-bold flex items-center gap-1">
        ← Back
      </button>
            ...
            {row.map((key, ki) => (
              <button
                ...
                style={{
                  background: key === 'del'
                    ? 'rgba(255,69,58,0.3)'
                    : key ? 'rgba(255,255,255,0.1)' : 'transparent',
                }}
              >
                {key === 'del' ? '⌫' : key}
              </button>
